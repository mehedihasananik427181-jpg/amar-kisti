...
# মূল অ্যাপ ফাংশনসমূহ (আলাদা আলাদা ভাগ করা)
def show_dashboard():
...
def collect_savings():
    ...
    if not data["members"]:
        st.warning("কোনো সদস্য নেই। আগে সদস্য তৈরি করুন।")
        return
...
def collect_loan():
    ...
    with col2:
        if st.button("🔥 আর্লি সেটেলমেন্ট (ঋণ ক্লোজ করুন)"):
            ...
            if loan_date_str:
                days_passed = max((datetime.now() - datetime.strptime(loan_date_str, '%Y-%m-%d %H:%M:%S')).days, 1)
                new_interest = principal * (info.get("loan_rate", 10) / 100) * (days_passed / 365)
                discounted_total = principal + new_interest
                data["members"][phone]["history"].append(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - আর্লি সেটেলমেন্টে লোন ক্লোজ। পার হওয়া দিন: {days_passed}। মোট পরিশোধ: {round(discounted_total, 2)} টাকা।")
                data["members"][phone]["loan_principal"] = 0.0
                data["members"][phone]["loan_interest"] = 0.0
                data["members"][phone]["loan_type"] = "নাই"
                save_data(data)
                st.success(f"🎉 সফল! আর্লি সেটেলমেন্ট মূল্যে ({round(discounted_total, 2)} টাকা) লোন সম্পূর্ণ ক্লোজ হয়েছে।")
                st.rerun()
...
def show_statement():
    ...
    st.markdown(f"### 👤 নাম: {info['name']} | 📱 মোবাইল: {phone}")
    st.write(f"💰 **মোট সঞ্চয়:** {info.get('savings', 0.0)} টাকা")
    st.write(f"📉 **বকেয়া আসল ঋণ:** {round(info.get('loan_principal', 0.0), 2)} টাকা")
    st.write(f"📊 **বকেয়া সুদ:** {round(info.get('loan_interest', 0.0), 2)} টাকা")
    st.write("---")
    for log in reversed(info.get("history", [])):
        st.markdown(f"📌 {log}")

# মূল এক্সিকিউশন ইন্টারফেস
